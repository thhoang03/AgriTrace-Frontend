  ...
  return (
    <div className="space-y-6 p-6 pb-10">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm font-medium text-green-700">Business intelligence</p>
          <h1 className="text-2xl font-bold text-gray-900">Analytics dashboard</h1>
          <p className="mt-1 text-sm text-gray-500">Monitor batch distribution, processing performance, and traceability.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
        {distribution.isLoading ? <LoadingCard /> : <section className={cardClass}>
          <div className="mb-5"><h2 className="font-semibold text-gray-900">Batch distribution</h2><p className="mt-1 text-xs text-gray-500">Current batches by lifecycle status</p></div>
          ...
          </div> : <p className="py-16 text-center text-sm text-gray-400">No distribution data available.</p>}
        </section>}

        {processing.isLoading ? <LoadingCard /> : <section className={cardClass}>
          <div className="mb-5 flex items-start justify-between"><div><h2 className="font-semibold text-gray-900">Processing time</h2><p className="mt-1 text-xs text-gray-500">Average hours by event type</p></div><Clock3 className="h-5 w-5 text-green-700" /></div>
          {stages.length ? <ResponsiveContainer width="100%" height={220}><BarChart data={stages} layout="vertical" margin={{ left: 12, right: 12 }}><CartesianGrid stroke="#f0f0f0" horizontal={false} /><XAxis type="number" tick={{ fontSize: 11 }} /><YAxis type="category" dataKey={stages[0].eventTypeCode ? "eventTypeCode" : "stage"} width={90} tick={{ fontSize: 11 }} /><Tooltip /><Bar dataKey={stages[0].averageHours !== undefined ? "averageHours" : "avgHours"} name="Average hours" fill="#2E7D32" radius={[0, 6, 6, 0]} /></BarChart></ResponsiveContainer> : <p className="py-16 text-center text-sm text-gray-400">No processing data available.</p>}
        </section>}
      </div>

      <section className={cardClass}>
        <div className="mb-4"><h2 className="font-semibold text-gray-900">Traceback analysis</h2><p className="mt-1 text-xs text-gray-500">Find batches and organizations related to a source batch.</p></div>
        <form onSubmit={searchTraceback} className="flex max-w-xl gap-2"><input value={batchId} onChange={(event) => setBatchId(event.target.value)} placeholder="Enter batch ID" className="min-w-0 flex-1 rounded-xl border border-gray-200 px-4 py-2.5 text-sm outline-none focus:border-green-600 focus:ring-2 focus:ring-green-100" /><button disabled={!batchId.trim() || traceback.isFetching} className="inline-flex items-center gap-2 rounded-xl bg-green-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-green-800 disabled:cursor-not-allowed disabled:opacity-50"><Search className="h-4 w-4" /> Search</button></form>
        {tracebackId && traceback.isLoading && <p className="mt-5 text-sm text-gray-500">Loading traceback…</p>}
        {tracebackId && traceback.isError && <p className="mt-5 text-sm text-red-600">Unable to load traceback for this batch.</p>}
        {traceback.data?.data && <div className="mt-6 grid gap-6 md:grid-cols-2"><div><h3 className="mb-3 text-sm font-semibold text-gray-700">Affected batches</h3><div className="space-y-2">{((traceback.data.data as any).affectedBatches ?? []).map((item: any) => <div className="flex justify-between rounded-lg bg-gray-50 px-3 py-2 text-sm" key={item.batchId}><span className="font-medium text-gray-800">{item.batchCode}</span><span className="text-gray-500">{item.relationship}</span></div>)}</div></div><div><h3 className="mb-3 text-sm font-semibold text-gray-700">Related organizations</h3><div className="space-y-2">{((traceback.data.data as any).relatedOrganizations ?? []).map((item: any) => <div className="flex justify-between rounded-lg bg-gray-50 px-3 py-2 text-sm" key={item.organizationId}><span className="font-medium text-gray-800">{item.name}</span><span className="text-gray-500">{item.type}</span></div>)}</div></div></div>}
      </section>
    </div>
  );
}