...
export const organizationsApi = {
  ...
  create: async (data: {
    ...
    const response = await post<OrganizationCreatedData>(
      "/organizations",
      newRequest,
    );
    return {
      data: { organizationId: (response.data as any).organizationId ?? 0 },
    };
  },

  update: async (
    id: number | string,
    data: { name?: string; organizationTypeId?: string; address?: string },
  ) => {
    const newRequest: Partial<OrganizationRequest> = {};
    if (data.name) newRequest.name = data.name;
    if (data.organizationTypeId)
      newRequest.organizationTypeId = data.organizationTypeId;
