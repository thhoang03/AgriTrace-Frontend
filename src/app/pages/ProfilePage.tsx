              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-sm outline-none transition-all focus:border-green-400 resize-none"
                    style={{ background: "#F8FAF8" }}
                  />
                </div>
              </div>
                ...
                {[
                  ...
                  <div key={key} className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`relative w-10 h-5.5 rounded-full transition-colors ${notifs[key as keyof typeof notifs] ? "bg-green-500" : "bg-gray-300"}`}
                      style={{ height: 22, width: 40 }}
                    >
                      <span
                        className="absolute top-0.5 left-0.5 w-4.5 h-4.5 bg-white rounded-full shadow-sm transition-transform"
                        style={{ width: 18, height: 18, transform: notifs[key as keyof typeof notifs] ? "translateX(18px)" : "translateX(0)" }}
                      />
                    </button>
                  </div>
                ))}
            ...
            <div className="bg-white rounded-2xl p-5" style={{ boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
              ...
              <div className="flex gap-2">
                {["Light", "Dark", "System"].map((t, i) => (
                  <button
                    key={t}
                    className={`flex-1 py-2 rounded-xl text-xs font-semibold transition-colors ${i === 0 ? "text-white" : "bg-gray-100 text-gray-600"}`}
                    style={i === 0 ? { background: "#2E7D32" } : {}}
                  >
                    {t}
                  </button>
                ))}
